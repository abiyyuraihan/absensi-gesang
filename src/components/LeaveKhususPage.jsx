...
const LeaveKhususPage = () => {
  ...
  const fetchLeaves = useCallback(async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_DATABASE_URL}/api/report-khusus/leaves`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingLeave) {
        await axios.put(
          `${process.env.REACT_APP_DATABASE_URL}/api/report-khusus/leaves/${editingLeave.id}`,
          leaveData
        );
      } else {
        await axios.post(
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this leave?")) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_DATABASE_URL}/api/report-khusus/leaves/${id}`
        );
        await fetchLeaves();
      } catch (error) {
...
export default LeaveKhususPage;
